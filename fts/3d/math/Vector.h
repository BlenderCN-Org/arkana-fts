...
namespace FTS {
    ...
    File& operator<<(File& f, const Vector& v);
...
class Vector {
    friend File& operator<<(File& f, const Vector& v);
    friend File& operator>>(File& f, Vector& v);
...
#ifdef D_CAL3D_INTEGRATION
    Vector(const CalVector&);
    operator CalVector() const;
