...
// *****************************************************************
// Permet de cr�r la table de r��ence CRC32 compatible Winzip en swappant
...
// c'est utile pour le projet que je d�eloppe actuellemnt...d�ol�;)
...
unsigned long CRC32_Fichier(char *NomFichier, unsigned long *TailleFichier)
{
    ...
    unsigned long crc32_table[256];     // Table identique a celle utilis� dans
    ...
    // D�ut
    *TailleFichier = 0;         // Bon OK � sert �rien ;) c plus propre c tout
    // Ouverture Fichier. Si Echec alors Fonction Renvoi 0 et sort immediatement
    if(!(Hfich = fopen(NomFichier, "rb")))
        return 0;
    // R�up�e Taille du Fichier. Si Echec alors Fonction Renvoi 0 et sort immediatement
    if((*TailleFichier = FLength(NomFichier, 'b')) == (unsigned long)-1L)
        return 0;
    // G��ation table de r��ence CRC32
    ...
    // Lecture Compl�e du Fichier en m�oire allou� pr�edement
    if(fread(octet, sizeof(unsigned char), cmpt, Hfich) < cmpt) {       // Erreur M.... !
        free(octet);            // Lib�ation m�oire
        fclose(Hfich);          // Fermeture du Fichier
        return 0;               // Retourne 0 => Erreur
    }
    // Calcul du CRC32 du Fichier
    CRC32 = 0xFFFFFFFF;         // D�art avec tous les bits �1
    poct = octet;
