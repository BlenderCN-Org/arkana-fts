...
bool CalSaver::saveXmlCoreMaterial(const std::string& strFilename, CalCoreMaterial *pCoreMaterial)
{
  ...
  material.SetAttribute("VERSION",Cal::CURRENT_FILE_VERSION);

  // Ambient, diffuse etc. are also stored as propreties.
  for(std::map<std::string, std::string>::const_iterator i = pCoreMaterial->getPropreties().begin() ; i != pCoreMaterial->getPropreties().end() ; ++i) {
      material.InsertEndChild(TiXmlElement(i->first))->InsertEndChild(TiXmlText(i->second));
  }

  // And now the maps:
  for(std::vector<CalCoreMaterial::Map>::const_iterator i = pCoreMaterial->getVectorMap().begin() ; i != pCoreMaterial->getVectorMap().end() ; ++i) {
      TiXmlElement map("Map");
      map.SetAttribute("Type", i->mapType);
      material.InsertEndChild(map)->InsertEndChild(TiXmlText(i->strFilename));
  }
