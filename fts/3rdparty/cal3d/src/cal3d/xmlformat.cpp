...
static inline float CalValueFromXml(cal3d::TiXmlElement *elem)
{
  cal3d::TiXmlNode* node = elem->FirstChild();
  if(!node) {
    throw std::runtime_error("Invalid value definition");
  }
  cal3d::TiXmlText* valueData = node->ToText();
  if(!valueData || !valueData->Value()) {
    throw std::runtime_error("Invalid value definition");
  }

  return (float)atof(valueData->Value());
}

static inline CalCoreMaterial::Color CalColorFromXml(cal3d::TiXmlElement *elem)
{
  CalCoreMaterial::Color color;
  cal3d::TiXmlNode* node = elem->FirstChild();
  if(!node) {
    throw std::runtime_error("Invalid color definition");
  }
  cal3d::TiXmlText* colorData = node->ToText();
  if(!colorData || !colorData->Value()) {
    throw std::runtime_error("Invalid color definition");
  }

  return CalCoreMaterial::Color(colorData->Value());
}

static inline std::string CalStringFromXml(cal3d::TiXmlElement *elem)
{
  cal3d::TiXmlNode* node = elem->FirstChild();
  if(!node) {
    throw std::runtime_error("Invalid color definition");
  }
  cal3d::TiXmlText* stringData = node->ToText();
  if(!stringData) {
    throw std::runtime_error("Invalid color definition");
  }

  return std::string(stringData->Value());
}

static inline const char* CalXmlAttributeI(cal3d::TiXmlElement *elem, const char* attr)
{
    // Make several guesses on the case of the attribute.
    std::string myAttr(attr);

    // Original.
    const char* res = elem->Attribute(myAttr);
    if(res) return res;

    //  ATTRIBUTE
    std::transform(myAttr.begin(), myAttr.end(), myAttr.begin(), (int (*)(int))std::toupper);
    res = elem->Attribute(attr);
    if(res) return res;

    // attribute
    std::transform(myAttr.begin(), myAttr.end(), myAttr.begin(), (int (*)(int))std::tolower);
    res = elem->Attribute(attr);
    if(res) return res;

    // Attribute
    if(myAttr.size() > 0)
        myAttr[0] = std::toupper(myAttr.at(0));
    res = elem->Attribute(attr);
    return res;
}
...
CalCoreMeshPtr CalLoader::loadXmlCoreMesh(const void *dataSrc, const CalCoreSkeleton* skel)
{
  ...
  return loadXmlCoreMesh(doc, skel);
}
...
CalCoreMeshPtr CalLoader::loadXmlCoreMesh(const std::string& strFilename, const CalCoreSkeleton* skel)
{
...
CalCoreMeshPtr CalLoader::loadXmlCoreMesh(cal3d::TiXmlDocument & doc, const CalCoreSkeleton* skel)
{
  std::string strFilename = "";
  ...
  for(submeshId = 0; submeshId < submeshCount; ++submeshId)
  {
    ...
    for(vertexId = 0; vertexId < vertexCount; ++vertexId)
    {
      ...
      {
        if(norm->FirstChild() && norm->FirstChild()->ToText() && norm->FirstChild()->ToText()->Value()) {
          ReadTripleFloat( norm->FirstChild()->ToText()->Value(),  &Vertex.normal.x, &Vertex.normal.y, &Vertex.normal.z );
        }
        norm = norm->NextSiblingElement();
      }

      cal3d::TiXmlElement *vertColor = norm;
      ...
      for(influenceId = 0; influenceId < influenceCount; ++influenceId)
      {
        ...
        if(skel && Vertex.vectorInfluence[influenceId].boneId >= skel->getNumCoreBones())
        {
            CalError::setLastError(CalError::INVALID_FILE_FORMAT, __FILE__, __LINE__, strFilename);
            return 0;
        }
...
{
  cal3d::TiXmlElement* elem = 0;

  try {
    elem = doc.FirstChildElement();
    if(!elem) {
      throw std::runtime_error("No XML element");
    }

    // Compatibility for HEADER tags, and read the version.
    if(elem->Value() && (stricmp(elem->Value(),"HEADER")==0 || stricmp(elem->Value(),"MATERIAL")==0))
    {
      if(!CalXmlAttributeI(elem, "VERSION") || atoi(CalXmlAttributeI(elem, "VERSION")) < Cal::EARLIEST_COMPATIBLE_FILE_VERSION ) {
        throw std::runtime_error("Too old version");
      }
    }

    // (Case-insensitively) search for the MATERIAL element.
    while(elem && elem->Value() && (stricmp(elem->Value(),"MATERIAL")!=0)) {
      elem = elem->NextSiblingElement();
    }

    if(!elem)
      throw std::runtime_error("No material element");

    CalCoreMaterialPtr pCoreMaterial(new CalCoreMaterial);

    // Read all material propreties
    for(elem = elem->FirstChildElement() ; elem ; elem = elem->NextSiblingElement()) {
      if(stricmp(elem->Value(), "MAP") == 0) {
        CalCoreMaterial::Map map(CalStringFromXml(elem));
        if(CalXmlAttributeI(elem, "TYPE")) {
          map.mapType = CalXmlAttributeI(elem, "TYPE");
        }
        pCoreMaterial->addMap(map);
      } else {
        pCoreMaterial->setProprety(elem->Value(), CalStringFromXml(elem));
      }
    }

    doc.Clear();
    return pCoreMaterial.get();
  } catch(const std::runtime_error& e) {
    std::stringstream ss;
    ss << "at line " << (elem ? elem->Row() : 0) << ": " << e.what();
    CalError::setLastError(CalError::INVALID_FILE_FORMAT, __FILE__, __LINE__, ss.str());
    doc.Clear();
    return 0;
  }
}
