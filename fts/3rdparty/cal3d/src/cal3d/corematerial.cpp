...
CalCoreMaterial::CalCoreMaterial( const CalCoreMaterial& inOther )
  ...
  , m_propreties( inOther.m_propreties )
{
}
...
const std::vector<CalCoreMaterial::Map>& CalCoreMaterial::getVectorMap() const
{
 ...
 /*****************************************************************************/
/** Returns a proprety of the material.
  *
  * This function returns a certain proprety for this material.
  *
  * @param key The name of the proprety to get. Case-insensitive.
  * @return The value of the proprety.
  *****************************************************************************/

std::string CalCoreMaterial::getProprety(const std::string& key) const
{
  std::string lowerkey = key;
  for(std::string::iterator i = lowerkey.begin() ; i != lowerkey.end() ; ++i)
      *i = ::tolower(*i);

    std::map<std::string, std::string>::const_iterator i = m_propreties.find(lowerkey);
 ...
 /*****************************************************************************/
/** Returns the propreties map.
  *
  * This function returns the map that contains all propreties of the core material
  * instance.
  *
  * @return A reference to the propreties map.
  *****************************************************************************/

const std::map<std::string, std::string>& CalCoreMaterial::getPropreties() const
{
  return m_propreties;
}
...
void CalCoreMaterial::setAmbientColor(const CalCoreMaterial::Color& ambientColor)
{
  ...
  this->setProprety("Ambient", ambientColor.toString());
}
...
void CalCoreMaterial::setDiffuseColor(const CalCoreMaterial::Color& diffuseColor)
{
  ...
  this->setProprety("Diffuse", diffuseColor.toString());
}

 /*****************************************************************************/
/** Adds a specified map.
  *
  * This function adds a specified map in the core material instance.
  *
  * @param map The map that should be set.
  *
  * @return the Id of the added map.
  *****************************************************************************/

int CalCoreMaterial::addMap(const Map& map)
{
...
void CalCoreMaterial::setShininess(float shininess)
{
  ...
  std::stringstream ss;
  ss << shininess;
  this->setProprety("Shininess", ss.str());
}
...
void CalCoreMaterial::setSpecularColor(const CalCoreMaterial::Color& specularColor)
{
  ...
  this->setProprety("Specular", specularColor.toString());
}
...
void CalCoreMaterial::setUserData(Cal::UserData userData)
{
 ...
 /*****************************************************************************/
/** Stores or removes a proprety.
  *
  * This function stores a proprety in the core material instance. (Overwrites
  * existing ones with the same name.)
  * If the value parameter is an empty string, it removes the proprety.
  *
  * @param key The name of the proprety to store. Case-insensitive.
  * @param value The value of the proprety to store.
  *****************************************************************************/

void CalCoreMaterial::setProprety(const std::string& key, const std::string& value)
{
  std::string lowerkey = key;
  for(std::string::iterator i = lowerkey.begin() ; i != lowerkey.end() ; ++i)
      *i = ::tolower(*i);
  ...
  if(value.empty())
    m_propreties.erase(lowerkey);
  else
    m_propreties[lowerkey] = value;

  // caching cases
  if(lowerkey == "shininess") {
    std::stringstream ss(value);
    ss >> m_shininess;
  } else if(lowerkey == "specular") {
    m_specularColor = Color(value);
  } else if(lowerkey == "diffuse") {
    m_diffuseColor = Color(value);
  } else if(lowerkey == "ambient") {
    m_ambientColor = Color(value);
  }
}
