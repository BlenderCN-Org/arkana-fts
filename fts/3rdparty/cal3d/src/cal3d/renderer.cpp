...
int CalRenderer::getVertColorsAsStandardPixels( unsigned int *pVertexBuffer)
{
  ...
  for(vertexId = 0; vertexId < vertexCount; ++vertexId)
  {
    ...
    * pVertexBuffer =
      ...
      + 0xff000000;
    pVertexBuffer++;
    // Let's just do the same for other platforms. I really don't know what it
    // should be here but it is still better than this unimplemented that does
    // not allow anyone but windows users to compile!
    /// @todo fix this CalRenderer::getVertColorsAsStandardPixels on non-win32 !!
    // unimplemented();
#endif
  }
  return vertexCount;
