...
int CalCoreModel::loadCoreAnimation(const void* buffer)
{
  // the core skeleton has to be loaded already
...
int CalCoreModel::loadCoreMaterial(const void* buffer)
{
  // the core skeleton has to be loaded already
  if(m_pCoreSkeleton == 0)
  {
...
int CalCoreModel::loadCoreMesh(const std::string& strFilename)
{
  ...
  CalCoreMeshPtr pCoreMesh = CalLoader::loadCoreMesh(strFilename, m_pCoreSkeleton.get());
...
int CalCoreModel::loadCoreMesh(const void* buffer)
{
  // the core skeleton has to be loaded already
  if(m_pCoreSkeleton == 0)
  {
  ...
  // load a new core mesh
  CalCoreMeshPtr pCoreMesh = CalLoader::loadCoreMesh(buffer, m_pCoreSkeleton.get());
  if (!pCoreMesh) return -1;
...
bool CalCoreModel::loadCoreSkeleton(const void* buffer)
{
  // load a new core skeleton
