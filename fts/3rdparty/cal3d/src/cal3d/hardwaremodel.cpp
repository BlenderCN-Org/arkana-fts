  ...
  : m_selectedHardwareMesh(-1)
{
  ...
  m_pvVertexBuffer = NULL;
  m_pIndexBuffer = NULL;
  m_pvIndexBuffer = NULL;
  m_pNormalBuffer = NULL;
  m_pvNormalBuffer = NULL;
  m_pWeightBuffer = NULL;
  m_pvWeightBuffer = NULL;
  m_pMatrixIndexBuffer = NULL;
  m_pvMatrixIndexBuffer = NULL;
  int i;
  for( i = 0 ; i < 8 ; i++) {
    m_pTextureCoordBuffer[i]=NULL;
    m_pvTextureCoordBuffer[i]=NULL;
  }
  m_textureCoordNum=0;
  
  for( i = 0 ; i < 8 ; i++) {
    m_pTangentSpaceBuffer [i]=NULL; 
    m_pvTangentSpaceBuffer [i]=NULL; 
  }
...
void CalHardwareModel::setVertexBuffer( std::vector<float> *pVertexBuffer, int stride)
{
  m_pvVertexBuffer = pVertexBuffer;
  m_vertexStride = stride;
}
...
void CalHardwareModel::setIndexBuffer( std::vector<CalIndex> *pIndexBuffer)
{
  m_pvIndexBuffer= pIndexBuffer;
}
...
void CalHardwareModel::setNormalBuffer( std::vector<float> *pNormalBuffer, int stride)
{
  m_pvNormalBuffer = pNormalBuffer;
  m_normalStride = stride;
}
...
void CalHardwareModel::setWeightBuffer( std::vector<float> *pWeightBuffer, int stride)
{
  m_pvWeightBuffer = pWeightBuffer;
  m_weightStride = stride;
}
...
void CalHardwareModel::setMatrixIndexBuffer( std::vector<float> *pMatrixIndexBuffer, int stride)
{
  m_pvMatrixIndexBuffer = pMatrixIndexBuffer;
  m_matrixIndexStride = stride;
}
...
void CalHardwareModel::setTextureCoordNum(size_t textureCoordNum)
{
...
void CalHardwareModel::setTextureCoordBuffer(int mapId, std::vector<float> *pTextureCoordBuffer, int stride)
{
  if( 0 <= mapId && mapId < 8)
  {
    m_pvTextureCoordBuffer[mapId] = pTextureCoordBuffer;
    m_textureCoordStride[mapId] = stride;   
  } 
}
...
void CalHardwareModel::setTangentSpaceBuffer(int mapId, std::vector<float> *pTangentSpaceBuffer, int stride)
{
  if( 0 <= mapId && mapId < 8)
  {
    m_pvTangentSpaceBuffer[mapId] = pTangentSpaceBuffer;
    m_tangentSpaceStride[mapId] = stride;   
  } 
}
...
const CalQuaternion & CalHardwareModel::getRotationBoneSpace(int boneId, const CalSkeleton *pSkeleton) const
{
...
const CalVector & CalHardwareModel::getTranslationBoneSpace(int boneId, const CalSkeleton *pSkeleton) const
{
  const std::vector<CalBone *>& vectorBone = pSkeleton->getVectorBone();
...
bool CalHardwareModel::load(int baseVertexIndex, int startIndex,size_t maxBonesPerMesh)
{
  ...
  for(size_t mapId = 0; mapId < m_textureCoordNum; mapId++)
  {
...
bool CalHardwareModel::canAddFace(CalHardwareMesh &hardwareMesh, CalCoreSubmesh::Face & face,std::vector<CalCoreSubmesh::Vertex>& vectorVertex, size_t maxBonesPerMesh) const
{
...
int CalHardwareModel::addVertex(CalHardwareMesh &hardwareMesh, int indice, CalCoreSubmesh *pCoreSubmesh, size_t maxBonesPerMesh)
{
  int i=0;
...
int CalHardwareModel::addBoneIndice(CalHardwareMesh &hardwareMesh, int Indice, size_t maxBonesPerMesh)
{ 
  size_t i=0;
...
bool CalHardwareModel::loadDyn(int baseVertexIndex, int startIndex,size_t maxBonesPerMesh)
{
  if(m_pvVertexBuffer==NULL || m_pvIndexBuffer==NULL)
  {
    CalError::setLastError(CalError::INVALID_HANDLE, __FILE__, __LINE__);
    return false;   
  }
  ...
  for(size_t mapId = 0; mapId < m_textureCoordNum; mapId++)
  {
    if(m_pvTextureCoordBuffer[mapId]==NULL)
    {
      CalError::setLastError(CalError::INVALID_HANDLE, __FILE__, __LINE__);
      return false;         
    }  
  ...
  for(std::vector<int>::iterator meshIdIt = m_coreMeshIds.begin();meshIdIt != m_coreMeshIds.end(); meshIdIt++)
  {
    ...
    {     
      ...
      for( faceId =0 ;faceId<pCoreSubmesh->getFaceCount();faceId++)
      {
        if(canAddFace(hardwareMesh,vectorFace[faceId],vectorVertex,maxBonesPerMesh))
        {
          m_pvIndexBuffer->push_back(addVertexDyn(hardwareMesh,vectorFace[faceId].vertexId[0],pCoreSubmesh,maxBonesPerMesh));
          m_pvIndexBuffer->push_back(addVertexDyn(hardwareMesh,vectorFace[faceId].vertexId[1],pCoreSubmesh,maxBonesPerMesh));
          m_pvIndexBuffer->push_back(addVertexDyn(hardwareMesh,vectorFace[faceId].vertexId[2],pCoreSubmesh,maxBonesPerMesh));
          hardwareMesh.faceCount++;
        }
        else
        {
...
int CalHardwareModel::addVertexDyn(CalHardwareMesh &hardwareMesh, int indice, CalCoreSubmesh *pCoreSubmesh, size_t maxBonesPerMesh)
{

  for(int i=0 ; i < hardwareMesh.vertexCount && i < (int)m_vectorVertexIndiceUsed.size() ; ++i) {
      if(m_vectorVertexIndiceUsed[i] == indice)
          return i;
  }
  ...
  m_vectorVertexIndiceUsed.push_back(indice);
  
  m_pvVertexBuffer->push_back(vectorVertex[indice].position.x);
  m_pvVertexBuffer->push_back(vectorVertex[indice].position.y);
  m_pvVertexBuffer->push_back(vectorVertex[indice].position.z);

  if(m_pvNormalBuffer) {
    m_pvNormalBuffer->push_back(vectorVertex[indice].normal.x);
    m_pvNormalBuffer->push_back(vectorVertex[indice].normal.y);
    m_pvNormalBuffer->push_back(vectorVertex[indice].normal.z);
  }

  for(size_t mapId = 0; mapId < m_textureCoordNum; mapId++)
  {
    if( vectorvectorTextureCoordinate.size() > mapId) {
      m_pvTextureCoordBuffer[mapId]->push_back(vectorvectorTextureCoordinate[mapId][indice].u);
      m_pvTextureCoordBuffer[mapId]->push_back(vectorvectorTextureCoordinate[mapId][indice].v);
    } else {
      m_pvTextureCoordBuffer[mapId]->push_back(0);
      m_pvTextureCoordBuffer[mapId]->push_back(0);
    }
  ...
  for(size_t mapId = 0; mapId < 8 ; mapId++)
  {
    ...
    {
      if(vectorvectorTangentSpace.size() > mapId && pCoreSubmesh->isTangentsEnabled(mapId)) {
          m_pvTangentSpaceBuffer[mapId]->push_back(vectorvectorTangentSpace[mapId][indice].tangent.x);
          m_pvTangentSpaceBuffer[mapId]->push_back(vectorvectorTangentSpace[mapId][indice].tangent.y);
          m_pvTangentSpaceBuffer[mapId]->push_back(vectorvectorTangentSpace[mapId][indice].tangent.z);
          m_pvTangentSpaceBuffer[mapId]->push_back(vectorvectorTangentSpace[mapId][indice].crossFactor);
      } else {
          m_pvTangentSpaceBuffer[mapId]->push_back(0);
          m_pvTangentSpaceBuffer[mapId]->push_back(0);
          m_pvTangentSpaceBuffer[mapId]->push_back(0);
          m_pvTangentSpaceBuffer[mapId]->push_back(0);
      }
  ...
  for(size_t l=0 ; l<4 ; l++)
  {
    ...
    { 
      ...
      if(m_pvWeightBuffer) {
        m_pvWeightBuffer->push_back(vectorVertex[indice].vectorInfluence[l].weight);     
      }
      if(m_pvMatrixIndexBuffer) {
        m_pvMatrixIndexBuffer->push_back(newBoneId);      
      }
    }
    else
    {
      if(m_pvWeightBuffer) {
        m_pvWeightBuffer->push_back(0);     
      }
      if(m_pvMatrixIndexBuffer) {
        m_pvMatrixIndexBuffer->push_back(0);      
      }
    }
  }

  hardwareMesh.vertexCount++;
  return m_vectorVertexIndiceUsed.size()-1;
}
