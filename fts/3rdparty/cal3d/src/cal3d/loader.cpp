...
CalCoreMeshPtr CalLoader::loadCoreMesh(const std::string& strFilename, CalCoreSkeleton *skel)
{

  if(strFilename.size()>= 3 && stricmp(strFilename.substr(strFilename.size()-3,3).c_str(),Cal::MESH_XMLFILE_MAGIC)==0)
    return loadXmlCoreMesh(strFilename, skel);
  ...
  CalCoreMeshPtr coremesh = loadCoreMesh( streamSrc, skel );
...
CalCoreMeshPtr CalLoader::loadCoreMesh(std::istream& inputStream, CalCoreSkeleton *skel)
{
   //Create a new istream data source and pass it on
   CalStreamSource streamSrc(inputStream);
   return loadCoreMesh(streamSrc, skel);
}
...
CalCoreAnimationPtr CalLoader::loadCoreAnimation(const void* inputBuffer, CalCoreSkeleton *skel)
{
  if ( (memcmp( inputBuffer, "<?xml", 5 ) == 0) || (memcmp( inputBuffer, "<HEADER", 7 ) == 0) || (memcmp( inputBuffer, "<ANIMATION", 10 ) == 0) )
  {
    TiXmlDocument doc;
...
CalCoreMaterialPtr CalLoader::loadCoreMaterial(const void* inputBuffer)
{
	if ( (memcmp( inputBuffer, "<?xml", 5 ) == 0) || (memcmp( inputBuffer, "<HEADER", 7 ) == 0) || (memcmp( inputBuffer, "<MATERIAL", 9 ) == 0) )
	{
...
CalCoreMeshPtr CalLoader::loadCoreMesh(const void* inputBuffer, CalCoreSkeleton *skel)
{
	if ( (memcmp( inputBuffer, "<?xml", 5 ) == 0) || (memcmp( inputBuffer, "<HEADER", 7 ) == 0) || (memcmp( inputBuffer, "<MESH", 5 ) == 0) )
	{
		TiXmlDocument	doc;
		doc.Parse( static_cast<const char*>(inputBuffer) );
		if (doc.Error())
...
CalCoreSkeletonPtr CalLoader::loadCoreSkeleton(const void* inputBuffer)
{
	if ( (memcmp( inputBuffer, "<?xml", 5 ) == 0) || (memcmp( inputBuffer, "<HEADER", 7 ) == 0) || (memcmp( inputBuffer, "<SKELETON", 9 ) == 0) )
	{
		TiXmlDocument	doc;
		doc.Parse( static_cast<const char*>(inputBuffer) );
		if (doc.Error())
...
CalCoreMaterialPtr CalLoader::loadCoreMaterial(CalDataSource& dataSrc)
{
  ...
  for(mapId = 0; mapId < mapCount; ++mapId)
  {
    ...
    CalCoreMaterial::Map map(strName);
  ...
  bool bHasMaterialPropreties = (version >= FIRST_FILE_VERSION_WITH_MATERIAL_PROPRETIES);
  if(bHasMaterialPropreties) {
    // read the number of propreties
    int propCount;
    if(!dataSrc.readInteger(propCount) || (propCount < 0))
    {
      CalError::setLastError(CalError::INVALID_FILE_FORMAT, __FILE__, __LINE__);
      return 0;
    }
    ...
    {
      // read the proprety key and value
      std::string key, value;
      dataSrc.readString(key);
      dataSrc.readString(value);
      ...
      pCoreMaterial->setProprety(key, value);
    }
  }
...
CalCoreMeshPtr CalLoader::loadCoreMesh(CalDataSource& dataSrc, CalCoreSkeleton *skel)
{
  ...
  for(int submeshId = 0; submeshId < submeshCount; ++submeshId)
  {
    ...
    CalCoreSubmesh *pCoreSubmesh = loadCoreSubmesh(dataSrc, version, skel);
    if(pCoreSubmesh == 0)
    {
      return 0;
    }
...
CalCoreSubmesh *CalLoader::loadCoreSubmesh(CalDataSource& dataSrc, int version, CalCoreSkeleton *skel)
{
	if(!dataSrc.ok())
	{
		dataSrc.setError();
      ...
      if( hasVertexColors ) {
		...
		for(influenceId = 0; influenceId < influenceCount; ++influenceId)
		{
			...
			if(skel && vertex.vectorInfluence[influenceId].boneId >= skel->getNumCoreBones()) {
                CalError::setLastError(CalError::BONE_NOT_FOUND, __FILE__, __LINE__, "");
            }
		}
